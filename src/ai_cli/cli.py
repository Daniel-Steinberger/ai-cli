...
def _consume_leading_options(args: list[str]) -> tuple[str | None, list[str]]:
    """Pull a leading `--model X` (if present) off the front of the args."""
    model = None
    while args and args[0].startswith("--"):
        if args[0] in ("--model", "--model=") or args[0].startswith("--model="):
...
def main(argv: list[str] | None = None) -> int:
    ...
    model_override, rest = _consume_leading_options(argv)
    ...
    if m:
        ...
        return _explain(config, offset, instruction, console)
...
def _explain(config, offset: int, instruction: str, console: Console) -> int:
    try:
